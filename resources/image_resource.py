...
class LogoUpload(Resource):
    ...
    @jwt_required()
    def put(cls):
        """Uploads the Vendor's Logo image for logged-in Vendor:
        ...
        After which we'll upd the new one"""
        ...
        if image_helper.ext_checker(image_helper.get_extension(data["image"])):
            ...
            # function will return the extension for filename passed
        ...
        # we find by extension because it could be in any format, meaning the full path cannot be consistent
        # function will return full path of image if found
        if logo_path:
            try:
                os.remove(logo_path)
            except:
        ...
        try:
            ...
            basename = image_helper.get_basename(path)  # get the name for display only
